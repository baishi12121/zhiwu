...
// 页面加载 — 通过 onLoad options 获取页面参数（比 defineProps 更可靠）
onLoad((options) => {
  if (options?.id) {
    addressId.value = options.id
    uni.setNavigationBarTitle({ title: '修改地址' })
    getMemberAddressByIdData(options.id)
  } else {
...
/** 从当前表单提取提交数据（去除 Object.assign 引入的 id 等多余字段） */
...
const onSubmit = async () => {
  try {
    ...
    if (addressId.value) {
      // 修改地址请求
      await putMemberAddressByIdAPI(addressId.value, data)
    } else {
      // 新建地址请求
      await postMemberAddressAPI(data)
    }
    // 成功提示
    uni.showToast({ icon: 'success', title: addressId.value ? '修改成功' : '添加成功' })
    // 返回上一页
    setTimeout(() => {
      uni.navigateBack()
    }, 400)
