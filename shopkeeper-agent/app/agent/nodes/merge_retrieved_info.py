    ...
    try:
        ...
        for retrieved_metric_info in retrieved_metric_infos:
            for relevant_column in retrieved_metric_info.relevant_columns:
                if relevant_column not in retrieved_column_infos_map:
                    ...
                    # 防御：metric 配置可能引用了不存在的字段，跳过避免 None 污染上下文
                    if column_info is None:
                        logger.warning(
                            f"指标 {retrieved_metric_info.name} 依赖的字段 {relevant_column} 在 meta 库中不存在，已跳过"
                        )
                        continue
        ...
        for retrieved_value_info in retrieved_value_infos:
            ...
            if column_id not in retrieved_column_infos_map:
                ...
                # 防御：取值召回可能命中已删除的字段，跳过避免 None 访问
                if column_info is None:
                    logger.warning(
                        f"取值召回命中的字段 {column_id} 在 meta 库中不存在，已跳过"
                    )
                    continue
        ...
        for table_id, column_infos in table_to_columns_map.items():
            ...
            # 防御：table_id 可能指向已删除的表，跳过避免 None 访问
            if table_info is None:
                logger.warning(
                    f"字段所属的表 {table_id} 在 meta 库中不存在，已跳过该表的所有字段"
                )
                continue
            columns = [
                ColumnInfoState(
